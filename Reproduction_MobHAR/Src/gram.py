...
def select_model(args,source,target):
    ...
    subexpFloder= 'checkpoint'+"/"+subexp+ '2' + "/" 
    ...
    for i in range(11, 21): 
        ...
        if os.path.exists(model_path):
            ...
            # --- Remap and Soft Load Logic ---
            new_dicts = {}
            for k, v in model_dicts.items():
                new_k = k.replace("transformer.", "encoder.")
                new_k = new_k.replace("fc.", "classifier.lin0.")
                new_k = new_k.replace("linear.", "classifier.lin0.")
                new_dicts[new_k] = v
            
            model.load_state_dict(new_dicts, strict=False)
            model = model.to(device)
            model.train() 
            J_t_median = eval_gram(model, args, target)
            J_t.append([J_t_median, model_path])

    # Handling the 'bestval' model
    classifier = fetch_classifier(args, input=args.encoder_cfg.hidden, output=args.activity_label_size)
    model = CompositeClassifier(args.encoder_cfg, classifier=classifier) 
    device = get_device(args.g)
    ...
    if os.path.exists(model_path):
        print(f"Loading bestval: {model_path}")
        model_dicts = torch.load(model_path, map_location=device)
    ...
    if not J_list:
        print("Error: No valid models found in checkpoint directory!")
        return None
