...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        # Interpolate positional embeddings if sequence length differs
        max_len = self.pos_embed.weight.size(0)   # original max length from checkpoint
        if seq_len != max_len:
            # Reshape: (max_len, hidden) -> (1, hidden, max_len)
            pos_embed_weights = self.pos_embed.weight.transpose(0, 1).unsqueeze(0)
            # Interpolate to target length
            pos_embed_weights = F.interpolate(pos_embed_weights, size=seq_len, mode='linear', align_corners=False)
            # Back to (1, seq_len, hidden)
            pos_embed = pos_embed_weights.squeeze(0).transpose(0, 1).unsqueeze(0)
            e = e + pos_embed
        else:
...
class CNN_UCI(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, channels, seq_len) – adjust if needed
        for layer in self.layers:  # assuming self.layers is a Sequential
            x = layer(x)
        return x
